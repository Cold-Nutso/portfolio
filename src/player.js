...
let umbraHbDims = [
    [ sprLength * 12/32, sprLength * 15/32, sprLength * 10/32, sprLength * 8/32 ],
    [ sprLength * 16/32, sprLength * 13/32, sprLength *  8/32, sprLength * 10/32 ]
];
let umbra = new NPC("umbraborn", 8, umbraHbDims, sprLength);

let umbraAltHbDims = [
    [ sprLength * 14/32, sprLength * 16/32, sprLength *  9/32, sprLength *  7/32 ],
    [ sprLength * 14/32, sprLength * 16/32, sprLength *  9/32, sprLength *  7/32 ]
];
let umbraAlt = new NPC("umbraborn_alt", 4, umbraAltHbDims, sprLength);

let knuckHbDims = [
    [ sprLength * 12/32, sprLength * 30/32, sprLength * 10/32, sprLength *  2/32 ],
    [ sprLength * 12/32, sprLength * 30/32, sprLength * 10/32, sprLength *  2/32 ]
];
...
dragElement(umbra);
dragElement(umbraAlt);

// Not entirely sure how this works yet
function dragElement(elmnt) {
  let pos1 = 0, pos2 = 0, pos3 = 0, pos4 = 0;
    
    // Move the char from the hitbox
    elmnt.hitbox.element.onmousedown = dragMouseDown;

  function dragMouseDown(e) {

    elmnt.hitbox.element.style.cursor = "grabbing";
...
window.setInterval(function() { gameLoop(); }, (1000/60) /*60fps*/);
...
function gameLoop() {
    ...
    else { // Nothing of significance is pressed
      knuck.input("NULL");
    }
...
document.addEventListener(
  ...
  function keyFunctions(key) {
    if (key.code == "keyA") {       // Left arrow moves card left   
      fileThrough(-1);
    }
    else if (key.code == "keyD") { // Right arrow moves card right
      fileThrough(1);
    }
    else if (key.code == "Escape")       // Escape exits showcase
      if (showcase.style.display == "block")
        showcase.style.display = "none";
    else if (key.code == "keyD")         // D toggles debug mode
      debugMode = !debugMode;
  }
);
